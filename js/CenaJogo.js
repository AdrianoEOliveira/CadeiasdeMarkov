...
export default class CenaJogo extends Cena {
        ...
        for (let gridI = 0; gridI < tamanhoGrid; gridI++) {
          for (let gridJ = 0; gridJ < tamanhoGrid; gridJ++) {
            ...
            if (gridI == tamanhoGrid - 1 && gridJ < tamanhoGrid - 1) {
              for (let l = gridI * separadorl + 2; l < this.LINHAS - 2; l++) {
                for (
    ...
    for (let l = 0; l < this.LINHAS; l++) {
      for (let c = 0; c < this.COLUNAS; c++) {
        ...
        if (l == 0 || l == this.LINHAS - 1 || c == 0 || c == this.COLUNAS - 1) {
          ...
          if (l >= 1 && c >= 1 && (l == x || y == c)) mapa.tiles[l][c] = Parede;
        }
      }
    }
  ...
  modeloDivide4partes(mapa) {
    ...
    let x = [
      Math.floor(this.LINHAS / 4),
      Math.floor(this.LINHAS / 2),
      Math.floor((3 * this.LINHAS) / 4),
    ];
    let y = [
      Math.floor(this.COLUNAS / 4),
      Math.floor(this.COLUNAS / 2),
      Math.floor((3 * this.COLUNAS) / 4),
    ];
  ...
  modeloDivide8partes(mapa) {
    ...
    let n = this.LINHAS;
    let m = this.COLUNAS;
    let linhasDivisoriasHorizontais = [
      Math.floor(n / 8),
      Math.floor(n / 4),
      Math.floor((3 * n) / 8),
      Math.floor(n / 2),
    ...
    let colunasDivisoriasVerticais = [
      Math.floor(m / 8),
      Math.floor(m / 4),
      Math.floor((3 * m) / 8),
      Math.floor(m / 2),
